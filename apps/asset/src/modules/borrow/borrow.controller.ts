...
export class BorrowController {
  ...
  @Get('my-borrow')
  async listMine(@CurrentUser() userId: number, @Query() query: MyBorrowQueryDto): Promise<{ items: unknown[]; agentShared: unknown[]; total: number }> {
    await assertFunctionAccess(this.prisma.client, userId, MY_BORROW_FUNCTION_CODE);
    return this.borrow.listMine(userId, query);
  }
  ...
  @Get('borrow-records')
  async listHistory(@CurrentUser() userId: number, @Query() query: BorrowHistoryQueryDto): Promise<{ items: unknown[]; total: number }> {
    ...
    return this.borrow.listHistory(query, departmentIds);
  }
}
