/**
 * asset OpenAPI 产物生成（主 PRD §9.5/§9.6；构建期生成，不承载运行时 UI）。
 *
 ...
 * `docs/api-documentations/openapi/asset.openapi.json`；
...
const OUTPUT_PATH = resolve(__dirname, '../../../../docs/api-documentations/openapi/asset.openapi.json');
...
/** 公开路由（与控制器 @Public() 标注一一对应；asset 无公开业务路由，仅健康探针） */
...
async function generate(): Promise<string> {
  ...
  try {
    ...
    const config = new DocumentBuilder()
      .setTitle('WBME asset API')
      .setDescription(
        'asset 部署单元（资产系统）OpenAPI 契约。构建期生成（主 PRD §9.5）：' +
...
async function main(): Promise<void> {
  ...
  if (check) {
    ...
    if (existing !== output) {
      console.error(`[openapi] 产物与提交文件不一致：${OUTPUT_PATH}\n[openapi] 请执行 pnpm --filter @wbme/asset openapi:generate 重新生成并提交`);
      process.exit(1);
    }
