...
/**
 * asset 操作日志与幂等执行共享工具（主 PRD §3.3；T7 建立）。
 *
 * 与 platform-core / hr 的操作日志工具同构：重要写操作在业务事务内写入
 * asset.operation_logs，以「操作者 + 系统(ASSET) + 幂等作用域 + 幂等键」部分唯一约束
 * （asset 迁移 20260809120000_add_operation_logs_idempotency_unique 已建）为唯一事实；
...
/** 操作人上下文（操作日志快照 + 部门快照） */
export interface AssetOperationLogOperator {
  id: number;
  name: string;
 ...
 *
 * @param prisma asset Prisma 客户端
 * @param operatorId 操作人 id
 * @returns 操作人上下文
 * @throws UNAUTHORIZED 操作人不存在或已删除
 */
export async function loadAssetOperationLogOperator(
  prisma: PrismaClient,
  operatorId: number,
): Promise<AssetOperationLogOperator> {
...
/**
 * 写入 asset 操作日志（只追加；与业务写入同事务，业务回滚日志同步回滚）。
 * requestId 取当前请求上下文。
 *
 * @param tx 事务客户端
 * @param entry 日志内容（feature = asset 目录中的功能编码）；携带幂等字段时该行同时充当幂等记录
 */
export async function writeAssetOperationLog(
...
/** 查询幂等记录（asset 日志表；操作者非空时 COALESCE 与精确匹配等价） */
async function findIdempotencyRecord(
  ...
  return prisma.assetOperationLog.findFirst({
    where: { operatorId, system: 'ASSET', idempotencyScope: scope, idempotencyKey: key },
...
export async function commitIdempotentOperation<T>(
  ...
  if (!idempotencyKey) {
    return prisma.$transaction(async (tx) => {
      ...
      await writeAssetOperationLog(tx, { operator, feature, actionType: outcome.actionType, summary: outcome.summary });
  ...
  try {
    return await prisma.$transaction(async (tx) => {
      const outcome = await run(tx);
      await writeAssetOperationLog(tx, {
        operator,
        feature,
 ...
 * @param options.scope 幂等作用域（如 `asset.transfer.create`）
