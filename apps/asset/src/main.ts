...
async function bootstrap(): Promise<void> {
  ...
  const queryLogInterceptor = createQueryOperationLogInterceptor(async (route, userId) => {
    ...
    const operator = await loadAssetOperationLogOperator(prisma.client, userId);
    const feature = getRequestContext()?.grantedFunction?.code ?? route;
    await prisma.client.$transaction((tx) =>
      writeAssetOperationLog(tx, {
        operator,
        feature,
        actionType: 'QUERY',
        summary: `查询了 ${route}`,
  ...
  const shutdown = async (signal: string): Promise<void> => {
    ...
    console.log(`[asset] 收到 ${signal}，开始优雅停机 ...`);
    const timer = setTimeout(() => {
      console.error('[asset] 优雅停机超时，强制退出');
      process.exit(1);
    }, SHUTDOWN_GRACE_MS);
    timer.unref();
    try {
      ...
      console.error('[asset] 停机失败', error);
      process.exit(1);
    }
  };
