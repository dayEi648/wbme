 ...
 * （hr 迁移 20260808082629_init 已建 operation_logs_idempotency_unique）为唯一事实；
...
/** 操作人上下文（操作日志快照 + 部门快照） */
export interface HrOperationLogOperator {
  id: number;
  name: string;
 ...
 *
 * @param prisma hr Prisma 客户端
 * @param operatorId 操作人 id
 * @returns 操作人上下文
 * @throws UNAUTHORIZED 操作人不存在或已删除
 */
export async function loadHrOperationLogOperator(
  prisma: PrismaClient,
  operatorId: number,
): Promise<HrOperationLogOperator> {
...
/**
 * 写入 hr 操作日志（只追加；与业务写入同事务，业务回滚日志同步回滚）。
 * requestId 取当前请求上下文。
 *
 * @param tx 事务客户端
 * @param entry 日志内容（feature = hr 目录中的功能编码）；携带幂等字段时该行同时充当幂等记录
 */
export async function writeHrOperationLog(
...
/** 查询幂等记录（hr 日志表；操作者非空时 COALESCE 与精确匹配等价） */
async function findIdempotencyRecord(
  ...
  return prisma.hrOperationLog.findFirst({
    where: { operatorId, system: 'HR', idempotencyScope: scope, idempotencyKey: key },
...
export async function commitIdempotentOperation<T>(
  ...
  if (!idempotencyKey) {
    return prisma.$transaction(async (tx) => {
      ...
      await writeHrOperationLog(tx, { operator, feature, actionType: outcome.actionType, summary: outcome.summary });
 ...
 * @param options.scope 幂等作用域（如 `hr.overtime.submit`）
