import { Button, Card, Descriptions, Drawer, Form, Input, Space, Table, Tabs, Typography } from 'antd';
...
const NAVIGATION: NavigationItem[] = [
  ...
  { key: 'overtime-apply', label: '加班申请', path: '/hr/overtime-apply', permission: ['overtime_apply', 'proxy_overtime'], group: '加班管理' },
  { key: 'overtime-mine', label: '我的加班', path: '/hr/overtime-mine', permission: 'overtime_apply', group: '加班管理' },
  { key: 'overtime-records', label: '加班历史', path: '/hr/overtime-records', permission: 'overtime_history', group: '加班管理' },
  { key: 'approval', label: '审批中心', path: '/hr/approval', permission: ['overtime_approval', 'org_structure'], group: '审批中心' },
  { key: 'settings', label: '系统设置', path: '/hr/settings', permission: 'hr_config', group: '系统设置' },
];
...
export default function HrPage() {
  ...
  const body = useMemo(() => {
    switch (section) {
      ...
      case 'departments':
        return <ResourcePage title="部门管理" service="hr" endpoint="/departments/tree" pageKey="hr-departments" columns={[...COMMON_COLUMNS, { key: 'parentName', title: '上级部门' }, { key: 'managerName', title: '负责人' }]} create={{ title: '新建部门', endpoint: '/departments', fields: [{ key: 'name', label: '部门名称', required: true, maxLength: 100 }, { key: 'parentId', label: '上级部门 ID', type: 'number' }, { key: 'sort', label: '排序', type: 'number' }] }} edit={{ title: '编辑部门', endpoint: (id) => `/departments/${id}`, fields: [{ key: 'name', label: '部门名称', maxLength: 100 }, { key: 'sort', label: '排序', type: 'number' }, { key: 'status', label: '状态', type: 'select', options: [{ label: '启用', value: 'ACTIVE' }, { label: '停用', value: 'DISABLED' }] }] }} batchDelete={{ endpoint: '/departments/delete', bodyKey: 'ids', previewEndpoint: '/departments/delete-preview', previewItem: (item) => ({ name: `#${String(item.id)}`, refs: `在职员工 ${String(item.activeEmployees ?? 0)} 人；现存资产 ${String(item.assetCount ?? 0)} 个；待审批申请 ${String(item.pendingRequests ?? 0)} 条；职称规则引用 ${String(item.titleRuleRefs ?? 0)} 条` }) }} />;
      case 'positions':
        return <ResourcePage title="岗位管理" service="hr" endpoint="/positions?includeDisabled=true" pageKey="hr-positions" columns={[...COMMON_COLUMNS, { key: 'description', title: '说明' }, { key: 'allowSelfApply', title: '允许自助申请' }, { key: 'departments', title: '适用部门' }]} create={{ title: '新建岗位', endpoint: '/positions', fields: [{ key: 'name', label: '岗位名称', required: true, maxLength: 100 }, { key: 'description', label: '说明', type: 'textarea', maxLength: 500 }, { key: 'allowSelfApply', label: '允许自助申请', type: 'boolean' }, { key: 'departmentIds', label: '适用部门 ID（JSON）', type: 'textarea', maxLength: 1000 }], transform: (values) => ({ ...values, departmentIds: parseIds(values.departmentIds) }) }} edit={{ title: '编辑岗位', endpoint: (id) => `/positions/${id}`, fields: [{ key: 'name', label: '岗位名称', maxLength: 100 }, { key: 'description', label: '说明', type: 'textarea', maxLength: 500 }, { key: 'allowSelfApply', label: '允许自助申请', type: 'boolean' }, { key: 'status', label: '状态', type: 'select', options: [{ label: '启用', value: 'ACTIVE' }, { label: '停用', value: 'DISABLED' }] }, { key: 'sort', label: '排序', type: 'number' }] }} batchDelete={{ endpoint: '/positions/delete', bodyKey: 'ids', previewEndpoint: '/positions/delete-preview', previewItem: (item) => ({ name: `#${String(item.id)}`, refs: `在岗员工 ${String(item.assignedEmployees ?? 0)} 人；待审批申请 ${String(item.pendingRequests ?? 0)} 条；职称规则引用 ${String(item.titleRuleRefs ?? 0)} 条` }) }} />;
      case 'titles':
...
function OrganizationEmployees() {
  ...
  const detailItems = target ? [
    { label: '员工 ID', children: String(target.id ?? '—') },
