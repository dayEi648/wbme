...
const NAVIGATION: NavigationItem[] = [
  ...
  { key: 'consumables', label: '消耗品配置', path: '/asset/consumables', permission: 'asset_config' },
  { key: 'warehouses', label: '库位管理', path: '/asset/warehouses', permission: 'asset_config' },
...
function FixedAssets() {
  ...
  return <>
    <ResourcePage
      ...
      batchDelete={{ endpoint: '/assets/batch', bodyKey: 'ids' }}
...
function QrCodeManagement() {
  ...
  return <ResourcePage key={version} title="二维码管理" description="二维码仅作扫码入口，不授予任何业务权限；作废后不可恢复，重新生成只更换公开标识。" service="asset" endpoint="/qr-codes" pageKey="asset-qr-codes" columns={[...COMMON_COLUMNS, { key: 'targetType', title: '目标类型' }, { key: 'status', title: '二维码状态', render: (value: unknown) => <StatusTag value={value} /> }]} create={{ title: '生成二维码', fields: [{ key: 'targetType', label: '目标类型', type: 'select', required: true, options: [{ label: '固定资产', value: 'ASSET' }, { label: '库存条目', value: 'INVENTORY_ITEM' }, { label: '申领目录', value: 'SCAN_CATALOG' }] }, { key: 'targetId', label: '目标 ID', type: 'number' }] }} rowActions={(row) => <Space size="small">{row.status === 'ACTIVE' ? <Popconfirm title="确认停用二维码？" onConfirm={() => void perform(Number(row.id), 'DISABLE')}><Button size="small">停用</Button></Popconfirm> : row.status === 'DISABLED' ? <Popconfirm title="确认恢复二维码？" onConfirm={() => void perform(Number(row.id), 'ENABLE')}><Button size="small">恢复</Button></Popconfirm> : null}<Popconfirm title="确认作废当前二维码并重新生成？" onConfirm={() => void perform(Number(row.id), 'REGENERATE')}><Button size="small" danger>重新生成</Button></Popconfirm></Space>} />;
}
...
function AssetConfig() {
  return <Space direction="vertical" size="large" style={{ width: '100%' }}>
    ...
    <ResourcePage title="资产分类" service="asset" endpoint="/categories" pageKey="asset-categories" columns={COMMON_COLUMNS} create={{ title: '新建分类', fields: [{ key: 'name', label: '名称', required: true, maxLength: 100 }, { key: 'parentId', label: '父分类 ID', type: 'number', required: true }] }} edit={{ title: '编辑资产分类', endpoint: (id) => `/categories/${id}`, fields: [{ key: 'name', label: '名称', required: true, maxLength: 100 }, { key: 'sort', label: '排序', type: 'number' }, { key: 'status', label: '状态', type: 'select', required: true, options: [{ label: '启用', value: 'ACTIVE' }, { label: '停用', value: 'DISABLED' }] }] }} batchDelete={{ endpoint: '/categories/batch', bodyKey: 'ids' }} />
    <ResourcePage title="业务字典" service="asset" endpoint="/dict-items" pageKey="asset-dicts" columns={[...COMMON_COLUMNS, { key: 'dictType', title: '类型' }]} create={{ title: '新建字典项', fields: [{ key: 'dictType', label: '字典类型', required: true }, { key: 'name', label: '名称', required: true, maxLength: 100 }] }} edit={{ title: '编辑字典项', endpoint: (id) => `/dict-items/${id}`, fields: [{ key: 'name', label: '名称', required: true, maxLength: 100 }, { key: 'sort', label: '排序', type: 'number' }, { key: 'status', label: '状态', type: 'select', required: true, options: [{ label: '启用', value: 'ACTIVE' }, { label: '停用', value: 'DISABLED' }] }] }} batchDelete={{ endpoint: '/dict-items/batch', bodyKey: 'ids' }} />
  </Space>;
}
