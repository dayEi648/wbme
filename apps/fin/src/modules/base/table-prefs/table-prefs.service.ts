...
/**
 * 用户表格偏好服务（T8 补全；F-9 同构于 B-5/H-18/A-30，账号维度读写，不写操作日志）。
 */
@Injectable()
export class TablePrefsService {
  ...
  async listFilterPresets(userId: number, pageKey: string): Promise<unknown> {
    const items = await this.prisma.client.userTablePref.findMany({
  ...
  async getColumnSetting(userId: number, pageKey: string): Promise<unknown> {
    const row = await this.prisma.client.userTablePref.findFirst({
  ...
  async createFilterPreset(userId: number, pageKey: string, dto: FilterPresetDto): Promise<unknown> {
    try {
      const row = await this.prisma.client.userTablePref.create({
        data: {
          userId,
          pageKey,
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002') {
        throw new BusinessException(frameworkErrors.VALIDATION_FAILED, { fields: [{ field: 'name', reason: '同名筛选预设已存在' }] });
      }
      throw error;
    }
  }
  ...
  async updateFilterPreset(userId: number, id: number, dto: FilterPresetDto): Promise<unknown> {
    const existing = await this.prisma.client.userTablePref.findFirst({
      where: { id, userId, prefType: 'FILTER_PRESET' },
    });
    if (!existing) {
      throw new BusinessException(frameworkErrors.RESOURCE_NOT_FOUND);
    }
    await this.prisma.client.userTablePref.update({
      where: { id },
  ...
  async deleteFilterPreset(userId: number, id: number): Promise<unknown> {
    ...
    await this.prisma.client.userTablePref.delete({ where: { id } });
    return { ok: true };
  }
