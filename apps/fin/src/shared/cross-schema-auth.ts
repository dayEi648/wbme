...
import { BusinessException, FINANCE_MAINTAIN_FUNCTION_CODE, FINANCE_VIEW_FUNCTION_CODE, frameworkErrors, type DataScope, type UserStatus } from '@wbme/contracts';
...
/** 功能访问上下文（对齐 backstage AuthorizationService.getFunctionAccess） */
...
/**
 * 查询 backstage.user_accounts 显示名（操作人姓名快照用）。
 *
 * @param prisma Prisma 客户端
 * @param userId 账号 id
 * @returns 姓名；不存在返回空串
...
/**
 * 功能访问上下文（跨 schema 镜像 backstage AuthorizationService.getFunctionAccess）。
 *
 * 功能注册与员工授权分别经 backstage.function_registry / backstage.function_grants
 * 只读视图读取（拥有模块 backstage），不再直连业务表（主 PRD §9.4）。
 *
 * @param prisma Prisma 客户端
...
/**
 * fin 只读访问断言（fin PRD §1：财务数据维护隐含包含财务数据查看）。
 * 列表/详情/利润分析只读/操作记录/导出共用：finance_view 或 finance_maintain 任一即可。
 */
export async function assertFinanceReadAccess(prisma: RawPrisma, userId: number): Promise<void> {
  const view = await getFunctionAccess(prisma, userId, FINANCE_VIEW_FUNCTION_CODE);
  if (view.registered && view.allowed) {
    if (!view.systemOpen) {
      throw new BusinessException(frameworkErrors.SYSTEM_NOT_OPEN, { system: view.systemName });
    }
    return;
  }
  const maintain = await getFunctionAccess(prisma, userId, FINANCE_MAINTAIN_FUNCTION_CODE);
  if (!maintain.registered || !maintain.allowed) {
    throw new BusinessException(frameworkErrors.RESOURCE_NOT_FOUND);
  }
  if (!maintain.systemOpen) {
    throw new BusinessException(frameworkErrors.SYSTEM_NOT_OPEN, { system: maintain.systemName });
  }
}

/**
 * fin 维护权限断言：必须持有「财务数据维护」（写操作唯一入口；隐含查看）。
 */
export async function assertFinanceMaintainAccess(prisma: RawPrisma, userId: number): Promise<void> {
  await assertFunctionAccess(prisma, userId, FINANCE_MAINTAIN_FUNCTION_CODE);
}

/**
 * fin 会话用户加载器：经 backstage.user_accounts 视图（替代直连 base.users）。
 */
@Injectable()
