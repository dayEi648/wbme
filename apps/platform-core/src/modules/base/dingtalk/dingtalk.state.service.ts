...
/** 钉钉授权用途（回调后按用途分流；BIND=已登录用户自助绑定，state 携带用户标识） */
export const DINGTALK_PURPOSES = ['LOGIN', 'REGISTRATION', 'ACTIVATION', 'RESET', 'BIND'] as const;
...
export interface DingtalkStateData {
  ...
  flowId?: string;
  /** BIND 用途的绑定目标用户标识（授权发起时已由会话守卫确认登录态） */
  userId?: number;
}
...
export class DingtalkStateService {
  ...
  /** 签发一次性 state 并返回；流程类用途需传入已校验的流程会话标识，BIND 用途传入绑定目标用户标识 */
  async issue(purpose: DingtalkPurpose, flowId?: string, userId?: number): Promise<string> {
    const state = randomBytes(32).toString('base64url');
    const data: DingtalkStateData = { purpose, flowId, userId };
