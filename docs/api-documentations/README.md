...
Prisma 与 SQL 列表端点在收到自定义 `sorts` 时会自动追加唯一 id 作为末级排序兜底
（Prisma 为 `{ id: 'desc' }`，SQL 为注册的 id 列 `DESC`），保证同值行翻页边界稳定（主 PRD §9.5）。
...
恢复确认 `POST /api/v1/restores/confirm` 使用独立 330 秒路由级时限（紧急备份等待上限 300 秒
+ 缓冲），Nginx 对该路径单独放宽读超时至 360 秒，避免长任务被普通 HTTP 超时截断。
