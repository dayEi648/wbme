...
| `PUT` | `/projects/deleted/restore` | 已删除项目批量恢复（1～100 个；全有或全无；保留原 ID/业务键/数据与操作历史；任一不存在或未删除整批回滚并返回失败明细） |
| `DELETE` | `/projects/batch` | 批量软删除（全有或全无；任一不存在或已删除整批回滚并返回失败明细；已删除项目不进入正常列表/筛选/统计/导出） |
