...
- 失败：`INVALID_CREDENTIALS`(401) / `ACCOUNT_LOCKED`(401，登录保护锁定，统一"尝试过多"提示) / `ACCOUNT_DEACTIVATED`(422) / `ACCOUNT_PENDING_ACTIVATION`(422) / `RATE_LIMITED`(429)
...
登录态。删除当前会话 + LOGOUT 安全日志 + 清 Cookie。入参 `IdempotentDto`。
...
公开 + 限流。入参 `{ token }`（M3 超管生成的 fragment 凭证）。成功同 A12（发流程 Cookie）。

### A11 换绑确认 `POST /auth/rebind/confirm`
换绑流程 Cookie。入参 `IdempotentDto`。
- 单事务：旧绑定 UNBOUND（历史保留）+ 新绑定 BOUND（新 unionId 未绑定）+ 手机号同步（新号码被占用 → 拒绝换绑）+ session_version 递增
- 成功：`{ ok: true }`（全会话失效，重新登录）
- 失败：`DINGTALK_ALREADY_BOUND` / `PHONE_TAKEN` / `FLOW_SESSION_INVALID` / `BINDING_NOT_FOUND`
- 安全事件：BINDING_CHANGED_COMPLETED（发起/失败由 A4/校验路径记录）
...
- 成功：`{ activationUrl }`（`PUBLIC_ORIGIN/activate#<token>`，凭证放 fragment，库中只存 SHA-256）
...
### M3 生成换绑邀请 `POST /users/{id}/rebind-invitations`
仅超管。入参 `IdempotentDto`。目标 ACTIVE 且有有效绑定。
- 成功：`{ rebindUrl }`；失败：`USER_NOT_ACTIVE` / `BINDING_NOT_FOUND` / `FORBIDDEN`
...
- 双时限：空闲超时（滑动续期，仅"有效交互" X-WBME-Active 头续期）+ 绝对过期；记住我仅延长时限
- 失效机制：改密/重置/换绑/注销 → session_version 递增，旧会话下次请求立即失效
- CSRF：双提交 Cookie `wbme_csrf` + 请求头 `X-WBME-CSRF-Token`（状态变更且携带会话 Cookie 时校验）
- 流程凭证：激活/重置/换绑凭证 = 32B 密码学随机值，库中只存 SHA-256，默认 7 天（系统设置可调）；URL fragment 承载，兑换后立即清除
- 手机号：平台标准格式 `+CC` + 纯数字；脱敏展示保留国家码与前 3 后 4（如 `+86 138****8000`）；唯一性仅"待激活+正常"间强制
- 安全日志：20 类事件逐条写 backstage.security_logs；不落密码/凭证/会话标识/邀请原文；手机号仅脱敏形式；写失败退 stderr 不阻断认证
