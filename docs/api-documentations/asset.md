...
| `GET` | `/categories` | 分类列表（顶级 + 一级子分类；状态过滤 + 分页） |
...
| `GET` | `/categories/delete-preview?ids=` | 删除前引用预览（逐分类现存资产数/品种数；引用不阻断删除） |
| `DELETE` | `/categories/batch` | 批量硬删除（主 PRD §2.6 确认式删除：预览后确认执行；资产/品种按删除前名称快照展示） |
| `GET` | `/dict-items` | 业务字典列表（dictType/status 筛选 + 分页） |
| `POST` | `/dict-items` | 新增字典项（幂等；同类型同名唯一） |
| `PUT` | `/dict-items/{id}` | 更新字典项（名称/排序/启停） |
| `GET` | `/dict-items/delete-preview?ids=` | 删除前引用预览（逐字典项当前业务引用数；引用不阻断删除） |
| `DELETE` | `/dict-items/batch` | 批量硬删除（主 PRD §2.6 确认式删除：预览后确认执行；业务与历史按删除前名称快照展示） |
...
| `GET` | `/warehouses/tree` | 库位树节点列表（状态过滤 + 分页） |
| `POST` | `/warehouses` | 创建库位（幂等；禁止父子循环） |
| `PUT` | `/warehouses/{id}` | 编辑库位（名称/排序/启停/移动节点；环校验；改名/移动只影响当前树，历史快照不追溯改写） |
| `GET` | `/warehouses/delete-preview?ids=` | 删除前引用预览（逐库位现存库存条目数/未结清借还数/待审批引用数；引用不阻断删除） |
| `DELETE` | `/warehouses/batch` | 批量硬删除（存在未删除子库位整批拒绝；主 PRD §2.6 确认式删除：预览后确认执行；批次/流水按删除前名称快照展示；停用后不能作为新入库/调拨目标，现存库存可调出） |
...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| `POST` | `/stock-in-requests` | 提交入库申请（幂等；行=品种+供应商/品牌/规格/库位+数量+可选单价；可填整单备注与申请时间，备注随审批详情展示；提交不占用库存；批准后按行建批次并增加库存，入库仅增） |
| `GET` | `/stock-in-requests/mine` | 本人入库申请历史 |
| `GET` | `/stock-in-requests` | 范围入库申请历史（审批状态/发起时间/发起人姓名筛选） |
| `POST` | `/stock-change-requests` | 提交库存变更申请（幂等；仅意外扣减，MVP 不支持增加库存；变更类型必填（字典 `CHANGE_TYPE`，系统初始化含「其他意外扣减」）；同一库存条目整单一次；可填整单备注，随审批详情展示；提交时锁定条目并原子占用，任一行不足整单不创建；批准后按批次 FIFO 扣减并生成流水，驳回/取消同事务释放占用） |
...
不设独立的个人记录汇集页（asset PRD §9）：申请人取消待审批的入口在各功能「历史记录」视图，
对待审批记录调用 `POST /approval-requests/{id}/cancel` 主动取消。
